...
def solution(N, Arr, K):
    ...
    Arr=input().split()
    K=int(input())
    r1=list(itertools.combinations(Arr, K))
    f=list(filter(lambda c: 'a' in c, r1))
    return ("{0:.3}".format(len(f) / len(r1)))
    # for each in itertools.combinations(Arr, K):
    #     r1.append(each)
    # return sum('a' in item for item in r1)/len(r1)
...
print(solution(4,['a', 'a', 'c', 'd', 'd' , 'a' , 'a' , 'gg', 'aa'], 2))